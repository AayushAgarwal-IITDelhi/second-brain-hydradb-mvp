...
class TestDecisionExtraction:
    ...
    def test_dedupes_repeated_decision(self):
        ...
        out = extract_decisions(
            "We agreed to use Railway. We agreed to use Railway."
        )
        assert len(out) == 1
...
class TestEntityExtraction:
    ...
    def test_dedupes_within_input(self):
        ...
        out = extract_entities(
            "Railway is fine. We moved to Railway last month."
        )
        railway_hits = [e for e in out
                        if e["content"] == "Railway"
...
class TestMemoryPersistence:
    ...
    def test_persists_workspace_scoped_rows(self):
        ...
        assert on_conflict == \
            "workspace_id,kind,content_hash,source_stable_key"
    ...
    def test_postgrest_error_logged_with_structured_body(self, caplog):
        ...
        records = [r for r in caplog.records
                   if r.message == "memory_persist_failed"]
...
class TestWorkspaceIsolation:
    ...
    def test_list_filters_on_workspace(self):
        ...
        order_mock.limit = MagicMock(return_value=MagicMock(
            execute=MagicMock(
        ...
        class _Chain:
            ...
            def eq(self, k, v):
                eq_calls.append((k, v)); return self
            def in_(self, k, v):
                in_calls.append((k, v)); return self
            def ilike(self, k, v):
                ilike_calls.append((k, v)); return self
            def order(self, *a, **k): return order_mock
        client = MagicMock()
        client.table = MagicMock(return_value=_Chain())
        with patch("memory_store.get_supabase", return_value=client):
            list_memories(workspace_id=WS1, kinds=["decision"],
                          query="kafka")
...
class TestMemoryAwareRetrieval:
    ...
    def test_memories_appear_in_recall_results(self):
        ...
        memory_card = next(
            s for s in sources if s.get("memory_kind") == "decision"
        )
    ...
    def test_source_filter_respects_memory_origin(self):
        ...
        kinds = [
            (s.get("source_kind"), s.get("memory_kind"))
    ...
    def test_memory_lookup_failure_does_not_block_answer(self):
        ...
        with patch(
            ...
            return_value={"chunks": [
                self._slack_chunk(
                    source_id="s1", text="kafka migration discussion",
                ),
            ]},
        ), patch(
            "memory_store.list_memories",
...
class TestMemoryFailureModes:
    ...
    def test_extract_and_persist_empty_input_zero(self):
        from memory_store import extract_and_persist
        assert extract_and_persist(
            workspace_id=WS1, source_kind="slack",
            source_stable_key=SLACK_KEY, source_timestamp=None,
            text="",
        ) == 0
